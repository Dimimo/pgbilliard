...
Route::prefix('venues')->group(function (): void {
    Route::get(
        'show/{venue}',
        fn ($venue) => view('pages.venues.show.[Venue]', [
            'venue' => \App\Models\Venue::query()->find($venue),
        ]),
    )->name('venues.show');
    Route::get(
        'edit/{venue}',
        fn ($venue) => view('pages.venues.edit.[Venue]', [
            'venue' => \App\Models\Venue::query()->find($venue),
        ]),
    )->name('venues.edit');
});
...
Route::middleware(['auth', 'admin'])
    ...
    ->group(function (): void {
        ...
        Route::get('players/overview', fn () => view('pages.admin.players.overview'))->name(
            'admin.players.overview',
        );
        Route::get('teams/create', fn () => view('pages.admin.teams.create'))->name(
            'admin.teams.create',
        );
        Route::get('venues/create', fn () => view('pages.admin.venues.create'))->name(
            'admin.venues.create',
        );
        ...
        Route::prefix('calendar')->group(function (): void {
            Route::get(
                'create/{season}',
                fn ($season) => view('pages.admin.calendar.create.[Season]', [
                    'season' => \App\Models\Season::query()->find($season),
                ]),
            )->name('admin.calendar.create');
            Route::get(
                'update/{season}',
                fn ($season) => view('pages.admin.calendar.update.[Season]', [
                    'season' => \App\Models\Season::query()->find($season),
                ]),
            )->name('admin.calendar.update');
            Route::get(
                'shift/{season}',
                fn ($season) => view('pages.admin.calendar.shift.[Season]', [
                    'season' => \App\Models\Season::query()->find($season),
                ]),
        ...
        Route::prefix('schedule')->group(function (): void {
            Route::get('/', fn () => view('pages.admin.schedule.index'))->name(
                'admin.schedule.index',
            );
            Route::get('create', fn () => view('pages.admin.schedule.create'))->name(
                'admin.schedule.create',
            );
            Route::get(
                'update/{format}',
                fn ($format) => view('pages.admin.schedule.update.[Format]', [
        ...
        Route::prefix('season')->group(function (): void {
            Route::get('create', fn () => view('pages.admin.seasons.create'))->name(
                'admin.seasons.create',
            );
            Route::get(
                'update/{season}',
                fn ($season) => view('pages.admin.seasons.update.[Season]', [
                    'season' => \App\Models\Season::query()->find($season),
                ]),
...
Route::prefix('forum')->group(function (): void {
    Route::prefix('posts')->group(function (): void {
        ...
        Route::get(
            'show/{post}',
            fn ($post) => view('pages.forum.posts.show.[Post]', [
                'post' => \App\Models\Forum\Post::query()->find($post),
            ]),
        )->name('forum.posts.show');
        Route::get(
            'edit/{post}',
            fn ($post) => view('pages.forum.posts.edit.[Post]', [
                'post' => \App\Models\Forum\Post::query()->find($post),
            ]),
    ...
    Route::prefix('comments')->group(function (): void {
        Route::get(
            'create/{post}',
            fn ($post) => view('pages.forum.comments.create.[Post]', [
                'post' => \App\Models\Forum\Post::query()->find($post),
            ]),
        )->name('forum.comments.create');
        Route::get(
            'edit/{comment}',
            fn ($comment) => view('pages.forum.comments.edit.[Comment]', [
...
Route::prefix('chat')->group(function (): void {
    ...
    Route::get(
        '{room}',
        fn ($room) => view('pages.chat.[.App.Models.Chat.ChatRoom]', [
            'room' => \App\Models\Chat\ChatRoom::query()->find($room),
        ]),
    )->name('chat.room');
    Route::get(
        'edit/{room}',
        fn ($room) => view('pages.chat.edit.[.App.Models.Chat.ChatRoom.]', [
            'room' => \App\Models\Chat\ChatRoom::query()->find($room),
        ]),
    )->name('chat.room.edit');
});
...
Route::middleware('guest')->group(function (): void {
    ...
    Route::get('forgot-password', fn () => view('pages.auth.forgot-password'))->name(
        'password.request',
    );
    Route::get('players/accounts', fn () => view('pages.players.accounts'))->name(
        'players.accounts',
    );
});
...
Route::get(
    'schedule/original',
    fn () => response()->download(public_path('day-schedule.pdf')),
)->name('schedule.original');
Route::get(
    'schedule/new',
    fn () => response()->download(public_path('reviewed_day_schedule.pdf')),
)->name('schedule.new');
...
Route::prefix('help')->group(function (): void {
    ...
    Route::get('competition-results', fn () => view('pages.help.competition-results'))->name(
        'help.results',
    );
...
Route::prefix('admin/help')
    ->middleware('auth')
    ->group(function (): void {
        Route::get('calendar', fn () => view('pages.admin.help.calendar'))->name(
            'admin.help.calendar',
        );
        Route::get('overview', fn () => view('pages.admin.help.overview'))->name(
            'admin.help.overview',
        );
        Route::get('schedule', fn () => view('pages.admin.help.schedule'))->name(
            'admin.help.schedule',
        );
        Route::get('season-structure', fn () => view('pages.admin.help.season-structure'))->name(
