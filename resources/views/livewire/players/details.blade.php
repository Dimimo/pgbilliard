<div>
    ...
    <x-forms.sub-title title="Individual Games and Results">
        @foreach($games as $game)
            <div class="p-2">
                @if ($new_date)
                    <div class="my-2 rounded-lg border border-indigo-400 bg-indigo-50 p-2 text-center font-bold">
                        {{ $game->event->date->date->format('jS \o\f M Y') }} @ {{ $game->event->venue->name }}
                        (<span class="font-normal">{{ $game->event->team_1->name }} vs {{ $game->event->team_2->name }}</span>)
                    </div>
                @endif
                @php
