                ...
                <tr class="h-8 hover:bg-blue-50" wire:key="{{ $post->id }}">
                    ...
                    <td>
                        @if ($post->comments_count > 0)
                            <x-svg.comments-solid color="fill-black" size="5" />
                            {{ $post->comments_count }}
                        @endif
                    </td>
                    @if (session('is_admin'))
                        <td>
                            <label class="flex justify-center">
                                <input
                                    type="checkbox"
                                    @if($post->is_sticky) checked @endif
                                    wire:click="toggle_sticky({{ $post->id }})"
                                />
                            </label>
                        </td>
                        <td>
                            <label class="flex justify-center">
                                <input
                                    type="checkbox"
                                    @if($post->is_locked) checked @endif
                                    wire:click="toggle_locked({{ $post->id }})"
                                />
                            </label>
                        </td>
                    @else
                    ...
                    @auth()
                        <td class="px-6 py-3">
                            @can('update', $post)
                                <a
                                    class="px-1"
                                    href="{{ route('forum.posts.edit', ['post' => $post]) }}"
                                    wire:navigate
                                >
                                    <x-forum.button-edit />
                                </a>
